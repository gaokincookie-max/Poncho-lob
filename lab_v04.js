...
const RULE_OFF_KEY='pochoLab.ruleOff.v04';
const ruleOff=new Set();
try{for(const id of JSON.parse(localStorage.getItem(RULE_OFF_KEY)||'[]'))ruleOff.add(id)}catch(_){}
function saveRuleOff(){try{localStorage.setItem(RULE_OFF_KEY,JSON.stringify([...ruleOff]))}catch(_){} updateRuleOffUi()}
...
function checkClause(c,ctx,self=ctx.primary||ctx.a,other=ctx.other||(self===ctx.a?ctx.b:ctx.a)){
 ...
 function layerAllowed(r){if(ruleOff.has(r.id))return false;const key=r.layer+'-'+r.result.toLowerCase();const cb=document.querySelector('[data-layer="'+key+'"]');if(!cb?.checked)return false;if(r.status==='unimplemented')return false;if(r.active)return true;return $('#include-disabled').checked&&r.status==='disabled'}
...
function ruleTable(s){const rules=DATA.behaviorRules.map(r=>({r,shadow:s.ruleShadow[r.id]||0,actual:s.ruleActual[r.id]||0})).filter(x=>x.shadow||x.actual).sort((a,b)=>b.shadow-a.shadow).slice(0,30);let h='<table class="tbl"><tr><th>条件</th><th>結果</th><th>層</th><th>裏成立</th><th>裏成立率</th><th>採用</th><th>個別</th></tr>';for(const x of rules){const risky=s.contacts&&x.shadow/s.contacts>.1?' class="warn"':'';const off=ruleOff.has(x.r.id);h+=`<tr${risky}><td>${x.r.id}${off?' <span class="off-badge">OFF</span>':''}<br><small>${x.r.condition}</small></td><td>${x.r.result}</td><td>${LABEL[x.r.layer]}</td><td>${x.shadow}</td><td>${pct(x.shadow,s.contacts)}</td><td>${x.actual}</td><td><button class="mini rule-toggle" data-rule-id="${x.r.id}">${off?'再有効化':'無効化'}</button></td></tr>`}return h+'</table>'}
...
window.POCHO_LAB_INTERNAL={createWorld,emptyStats,seedRandom,setSeed:(seed)=>{rng=seedRandom(seed)},getStats:()=>lastStats,isRuleDisabled:(id)=>ruleOff.has(id),setRuleDisabled,toggleRuleDisabled,getRuleOff:()=>[...ruleOff],pct,bucketTable,layerTable,ruleTable,groupTable,constants:{W,H,launchY,DT,T,PHYS,LABEL,LAYERS,COLORS,EXPRESSIONS}};
