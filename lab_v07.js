...
const RULE_OFF_KEY='pochoLab.ruleOff.v05';
const LAYER_SETTINGS_KEY='pochoLab.layerSettings.v05';
...
function defaultLayerEnabled(key){const [layer,result]=key.split('-');return !(result==='pop'&&(layer==='size'||layer==='expression'))}
...
function checkClause(c,ctx,self=ctx.primary||ctx.a,other=ctx.other||(self===ctx.a?ctx.b:ctx.a)){
 ...
 let currentShot=0;Events.on(engine,'collisionStart',ev=>{if(initializingBoard)return;for(const pair of ev.pairs){const a=pair.bodyA,b=pair.bodyB;if(a.pocho&&b.pocho)processContact(a,b,pair,currentShot,shots)}});Events.on(engine,'collisionActive',ev=>{for(const pair of ev.pairs){let b=null;if(pair.bodyA===topWall&&pair.bodyB?.pocho)b=pair.bodyB;else if(pair.bodyB===topWall&&pair.bodyA?.pocho)b=pair.bodyA;if(b){if(simNow-b.pocho.wall.lastAt>180){b.pocho.wall.top++;b.pocho.wall.last='top';b.pocho.wall.lastAt=simNow;b.pocho.wall.sequence.push('top');trimSeq(b.pocho.wall.sequence,8)}if(b.velocity.y<0||Math.abs(b.velocity.y)<2)Body.setVelocity(b,{x:b.velocity.x*.28,y:0})}}});
 function launchVector(vx,vy,desc=makeDescriptor()){if(currentShot>=shots)return null;currentShot++;const b=makeBody(desc,'shot');Composite.add(engine.world,b);bodies.add(b);Body.setVelocity(b,{x:vx,y:vy});stats.shots++;opts.onLaunch?.({body:b,shot:currentShot,desc,velocity:{x:vx,y:vy}});return b}
