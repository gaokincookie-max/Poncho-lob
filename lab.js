...
function checkClause(c,ctx,self=ctx.primary||ctx.a,other=ctx.other||(self===ctx.a?ctx.b:ctx.a)){
 ...
 function launch(){currentShot++;const b=makeBody(makeDescriptor());Composite.add(engine.world,b);bodies.add(b);let sx=0,sy=-1,spd=10;if(ai==='strong'){spd=rand(12.8,16.5);sx=rand(-.75,.75)}else if(ai==='weak'){spd=rand(3,8);sx=rand(-.55,.55)}else if(ai==='center'){spd=rand(8,14);const targetX=W/2+rand(-25,25),dx=targetX-b.position.x,dy=-220,s=Math.hypot(dx,dy);sx=dx/s;sy=dy/s}else if(ai==='crowd'&&bodies.size>1){let tx=W/2,ty=H*.3,best=-1;for(const x of bodies){if(x===b)continue;const near=getNearby(x.position.x,x.position.y,90,[b]).length;if(near>best){best=near;tx=x.position.x;ty=x.position.y}}const dx=tx-b.position.x,dy=ty-b.position.y,s=Math.max(1,Math.hypot(dx,dy));sx=dx/s;sy=dy/s;spd=rand(9,15)}else{spd=rand(5,16);sx=rand(-.8,.8)}if(ai!=='center'&&ai!=='crowd'){sy=-Math.sqrt(Math.max(.05,1-sx*sx))}Body.setVelocity(b,{x:sx*spd,y:sy*spd});stats.shots++}
 ...
 return{async run(){for(let s=1;s<=shots;s++){launch();for(let i=0;i<24;i++)step()}for(let i=0;i<180;i++)step();stats.games++;return bodies.size}}}
...
async function runAll(){stopFlag=false;$('#run').disabled=true;$('#stop').disabled=false;$('#csv').disabled=true;$('#summary-csv').disabled=true;rng=seedRandom($('#seed').value);const games=Math.max(1,Math.min(1000,+$('#games').value||1)),shots=+$('#mode').value,ai=$('#ai').value,stats=emptyStats();lastStats=stats;const start=performance.now();for(let g=0;g<games&&!stopFlag;g++){const w=createWorld(stats,shots,ai);await w.run();if(g%2===0){$('#bar').style.width=((g+1)/games*100)+'%';$('#status').textContent=`${g+1} / ${games} games ・ ${stats.contacts.toLocaleString()} contacts`;render(stats);await new Promise(r=>setTimeout(r,0))}}render(stats);lastLogs=stats.logs;$('#bar').style.width=(stopFlag?stats.games/games*100:100)+'%';$('#status').textContent=(stopFlag?'停止':'完了')+`：${stats.games} games / ${stats.shots.toLocaleString()} shots / ${stats.contacts.toLocaleString()} contacts / ${((performance.now()-start)/1000).toFixed(1)} sec`;$('#run').disabled=false;$('#stop').disabled=true;$('#csv').disabled=!stats.logs.length;$('#summary-csv').disabled=!stats.contacts}
...
setupToggles();$('#run').onclick=runAll;$('#stop').onclick=()=>stopFlag=true;$('#csv').onclick=logsCsv;$('#summary-csv').onclick=summaryCsv;
})();